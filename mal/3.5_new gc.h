...
size_t pik_mark_garbage(pickle_t vm);
...
static pik_object_t alloc_object(pickle_t vm, uint16_t type, uint16_t subtype) {
    ...
    if (vm->num_objects > vm->in_use_objects) {
        while (object != NULL) {
...
static void finalize(pickle_t vm, pik_object_t object) {
    ...
    switch (type_info(object->type) & CELL1_MASK) {
        ...
        case CELL1_OBJECT: object->cell1 = NULL; break;
    }
    switch (type_info(object->type) & CELL2_MASK) {
        ...
        case CELL2_OBJECT: object->cell2 = NULL; break;
    }
    switch (type_info(object->type) & CELL3_MASK) {
        case CELL3_EMPTY: break;
        case CELL3_OBJECT: object->cell3 = NULL; break;
    }
...
static void mark_object(pik_object_t object) {
    mark:
    ...
    switch (type_info(object->type) & CELL1_MASK) {
        ...
        case CELL1_OBJECT: mark_object(object->cell1); break;
    }
    switch (type_info(object->type) & CELL2_MASK) {
        case CELL2_EMPTY: break;
        case CELL2_FILE: break;
        case CELL2_OBJECT: mark_object(object->cell2); break;
    }
    switch (type_info(object->type) & CELL3_MASK) {
        case CELL3_EMPTY: break;
        case CELL3_OBJECT: mark_object(object->cell3); break;
    }
    // Mark properties
    PIK_DEBUG_PRINTF("%p->properties\n", (void*)object);
    mark_object(object->properties);
    // Mark classes
    // Tail-call optimize
...
static void finalize_all_unmarked(pickle_t vm) {
    pik_object_t object = vm->first_object;
    while (object != NULL) {
        ...
        if (object->flags & MARKBIT) {
            ...
            PIK_DEBUG_PRINTF("unmarked, finalizing\n");
            // Sweep the object
            finalize(vm, object);
        }
        object = object->next_object;
...
size_t pik_mark_garbage(pickle_t vm) {
    ...
    mark_object(vm->global_scope);
    mark_object(vm->dollar_function);
    mark_object(vm->type_protos);
    size_t start = vm->in_use_objects;
    finalize_all_unmarked(vm);
    size_t freed = start - vm->in_use_objects;
    vm->next_gc = vm->num_objects + 64;
    PIK_DEBUG_PRINTF("%zu new garbage, %zu objects remaining after gc, %zu total allocated\n", freed, vm->in_use_objects, vm->num_objects);
    return freed;
}
...
void pik_destroy(pickle_t vm) {
    ...
    pik_mark_garbage(vm);
    PIK_DEBUG_ASSERT(vm->in_use_objects == 0, "Garbage collection failed to finalize all objects");
    PIK_DEBUG_PRINTF("Freeing %zu objects\n", vm->num_objects);
    while (vm->first_object) {
        pik_object_t temp = vm->first_object;
        vm->first_object = temp->next_object;
        free(temp);
    }
...
void pik_delete_at_index(pik_object_t array, size_t i) {
    if (i + 1 < array->len) {
...
void pik_insert_at_index(pik_object_t array, size_t i, pik_object_t what) {
    if (i >= array->len) {
        while (i < array->len) pik_append(array, NULL);
        pik_append(array, what);
    } else {
        array->items = (pik_object_t*)realloc(array->items, (array->len + 1) * sizeof(pik_object_t));
        array->len++;
        memmove((void*)&array->items[i+1], (void*)&array->items[i], (array->len - i - 1) * sizeof(pik_object_t));
        array->items[0] = what;
    }
}

#define PIK_DONE(vm, scope, rval) do { scope->result = rval; return ROK; } while (0)
...
static int reduce_expression(pickle_t vm, pik_object_t self, pik_object_t func, pik_object_t expr, pik_object_t args, pik_object_t scope) {
    ...
    expr = scope->result;
    pik_insert_at_index(expr, 0, func);
    next_operator:
    ...
    pik_delete_at_index(expr, pos);
    pik_delete_at_index(expr, pos+1);
...
static int eval_expression(pickle_t vm, pik_object_t self, pik_object_t expr, pik_object_t args, pik_object_t scope) {
    ...
    if (is_macro(func)) {
        ...
        for (size_t i = 1; i < reduced->len; i++) {
